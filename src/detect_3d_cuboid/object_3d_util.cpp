...
// box_config_type  [configuration_id, vp_1_on_left_or_right]      cuboid struct has this field.
void get_cuboid_draw_edge_markers(  MatrixXi& edge_markers,             
                                    const Vector2d& box_config_type,    
                                    bool final_universal_object)        
{
...
void plot_image_with_cuboid_edges(  cv::Mat& plot_img, 
                                    const MatrixXi& box_corners_2d,     
                                    const MatrixXi& edge_markers)       
{
    MatrixXi line_markers(6,4); // each row is  BGR, line_thickness线段粗细
    line_markers << 0,0,255,3,      
                    0,0,255,0.3,    
                    0,255,0,3,      
                    0,255,0,0.3,    
                    255,0,0,3,      
...
Vector2d lineSegmentIntersect(  const Vector2d& pt1_start, const Vector2d& pt1_end,
                                const Vector2d& pt2_start, const Vector2d& pt2_end,
			                    bool infinite_line)
{
...
Eigen::MatrixXd VP_support_edge_infos(  Eigen::MatrixXd& VPs,                   
                                        Eigen::MatrixXd& edge_mid_pts,          
...
double box_edge_sum_dists(  const cv::Mat& dist_map,            
                            const MatrixXd& box_corners_2d,     
                            const MatrixXi& edge_pt_ids,        
...
double box_edge_alignment_angle_error(  const MatrixXd& all_vp_bound_edge_angles,   
                                        const MatrixXi& vps_box_edge_pt_ids,        
                                        const MatrixXd& box_corners_2d)             
{
...
// vps_box_edge_pt_ids: % six edges. each row represents two edges [e1_1 e1_2   e2_1 e2_2;...] of one VP
    ...
    for (int vp_id = 0; vp_id < vps_box_edge_pt_ids.rows(); vp_id++)
    {
        ...
        if (vp_bound_angles_valid.size() > 0) //  exist valid edges
        {
            ...
            {
                Vector2d two_box_corners_1 = box_corners_2d.col( vps_box_edge_pt_ids(vp_id, 2*ee_id) );     
                Vector2d two_box_corners_2 = box_corners_2d.col( vps_box_edge_pt_ids(vp_id, 2*ee_id+1) );   
                ...
                for (int i = 0; i < (int)vp_bound_angles_valid.size(); i++)
                {
                ...
                if (static_cast<bool>(print_once)) {
...
void fuse_normalize_scores_v2(  const VectorXd& dist_error,                    
                                const VectorXd& angle_error,        
                                VectorXd& combined_scores,              
                                std::vector<int>& final_keep_inds,  
			                    double weight_vp_angle,             
                                bool whether_normalize)             
{
...
void plane_hits_3d( const Matrix4d& transToWolrd,   
                    const Matrix3d& invK,           
                    const Vector4d& plane_sensor,   
                    MatrixXd pixels,                
                    Matrix3Xd& pts_3d_world)        
{
...
void getVanishingPoints(const Matrix3d& KinvR,  
                        double yaw_esti,        
                        Vector2d& vp_1,         
                        Vector2d& vp_2, 
                        Vector2d& vp_3)
...
void change_2d_corner_to_3d_object( const MatrixXd& box_corners_2d_float,   
                                    const Vector3d& configs,                
                                    const Vector4d& ground_plane_sensor,    
